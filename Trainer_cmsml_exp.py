...
if in_ipynb():
    ...
    #exec("from "+importConfig+" import modifydf")
...
for MVA in Conf.MVAs:
    ...
    if 'DNN' in MVA["MVAtype"]:
        ...
        try:
            ...
            X_test = sc.transform(X_test)
            #print(sc.mean_)
            with open(Conf.OutputDirName+"/"+MVA["MVAtype"]+"/"+MVA["MVAtype"]+"_"+"_scaler.txt","w") as f:
                for name,mean,var in zip(MVA["featuresgettr"],sc.mean_,sc.scale_):
                    f.write(f'{name} {mean} {var} \n')
            import pickle
            pickle.dump(sc, open(Conf.OutputDirName+"/"+MVA["MVAtype"]+"/"+MVA["MVAtype"]+"_"+"_scaler.pkl",'wb'))
        except:
            print("Data is not being scaled! Either no scaling option provided or scaling not found")
        ...
        modelDNN.save(Conf.OutputDirName+"/"+MVA["MVAtype"]+"/"+MVA["MVAtype"]+"_"+"modelDNN")
        #try:
        import cmsml as cmsml
        print("Trying_Saving")
        #tf.compat.v1.enable_eager_execution()
        cmsml.tensorflow.save_graph(Conf.OutputDirName+"/"+MVA["MVAtype"]+"/"+MVA["MVAtype"]+"_"+"modelDNN.pb.txt",modelDNN, variables_to_constants=True)
        cmsml.tensorflow.save_graph(Conf.OutputDirName+"/"+MVA["MVAtype"]+"/"+MVA["MVAtype"]+"_"+"modelDNN.pb",modelDNN, variables_to_constants=True)
        #tf.compat.v1.disable_eager_execution()
        #except:
        ...
        if len(Conf.Classes)>2:
            ...
            df_final.loc[TrainIndices,MVA["MVAtype"]+"QCD_pred"]=modelDNN.predict(X_train,batch_size=5000)[:, 2]
            df_final.loc[TestIndices,MVA["MVAtype"]+"QCD_pred"]=modelDNN.predict(X_test,batch_size=5000)[:, 2]


            # df_final.loc[TrainIndices,MVA["MVAtype"]+"_pred"]=df_final.loc[TrainIndices,MVA["MVAtype"]+"_pred"] - df_final.loc[TrainIndices,MVA["MVAtype"]+"QCD_pred"]
            # df_final.loc[TestIndices,MVA["MVAtype"]+"_pred"]=df_final.loc[TestIndices,MVA["MVAtype"]+"_pred"] - df_final.loc[TestIndices,MVA["MVAtype"]+"QCD_pred"] 
...
if hasattr(Conf, 'SigEffWPs')and len(Conf.SigEffWPs)>0:
    ...
    for MVA in Conf.MVAs:
        ...
        #EB_train.loc[EB_train[MVA["MVAtype"]+"QCD_pred"] > 0.7, MVA["MVAtype"]+"_predpass"] = 0
        ...
        #EB_train.loc[EB_train[MVA["MVAtype"]+"QCD_pred"] > 0.7, MVA["MVAtype"]+"_predpass1"] = 0
        ...
        #EB_test.loc[EB_test[MVA["MVAtype"]+"QCD_pred"] > 0.7, MVA["MVAtype"]+"_predpass"] = 0
